...
int main()
{
	...
	string path1 = "D:\\Mario\\Mario_Tileset.png",
		path2 = "D:\\Mario\\Jump.ogg",
		path3 = "D:\\Mario\\Mario_Theme.ogg";
	Texture tileSet;
	tileSet.loadFromFile(path1);
	...
	SoundBuffer buffer;
	buffer.loadFromFile(path2);
	Sound sound(buffer);

	Music music;
	music.openFromFile(path3);
	music.setLoop(true);
	music.play();
	...
	while (window.isOpen())
	{
		...
		if (Keyboard::isKeyPressed(Keyboard::Up)) {
			if (Mario.onGround) {
				Mario.dy = -0.2; Mario.onGround = false;  sound.play();
			}
			else Mario.dy -= 0.0003 * time;
		}
